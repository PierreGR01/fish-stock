// TASK-17 — Improved pedagogical messages
import { useEffect, useRef, useState } from 'react';
import { useFishStore, SIM_MESSAGES } from '../../store/fishStore';
import { PacificMap } from '../map/PacificMap';

const SIM_STEP_LABELS = ['Initialisation', 'Trajectory calculation', 'Results aggregation'];

// Map simPhaseIdx to relevant message indices
const PHASE_MSG_RANGES: [number, number][] = [
  [0, 1],  // init phase: msgs 0-1
  [2, 4],  // trajectory: msgs 2-4
  [5, 7],  // aggregation: msgs 5-7
];
...
export function SimulateScreen() {
  ...
  // TASK-17: message index follows simPhaseIdx
  useEffect(() => {
    const [min, max] = PHASE_MSG_RANGES[Math.min(simPhaseIdx, 2)] ?? [0, 1];
    setMsgIdx(prev => (prev >= min && prev <= max) ? prev : min);
  }, [simPhaseIdx]);

  useEffect(() => {
    if (done) return;
    tickRef.current = setInterval(() => { tickSimulation(); }, 80);
  ...
  useEffect(() => {
    msgRef.current = setInterval(() => {
      setMsgIdx(i => {
        const phase = useFishStore.getState().simPhaseIdx;
        const [min, max] = PHASE_MSG_RANGES[Math.min(phase, 2)] ?? [0, 1];
        const next = i + 1;
    ...
    <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <p style={{ fontFamily: 'var(--font-ui)', fontSize: 10, color: 'var(--text-lo)', textAlign: 'center', marginBottom: 20 }}>
          SEAPODYM spatial model · CMEMS input data
        </p>
          ...
          {SIM_STEP_LABELS.map((label, i) => {
            ...
            return (
              <div key={label} style={{ display: 'flex', alignItems: 'center' }}>
                {i > 0 && <div style={{ width: 20, height: 1, background: state === 'todo' ? 'var(--ink-500)' : 'var(--signal-ok)', opacity: 0.7 }} />}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{
                    width: 10, height: 10, borderRadius: '50%',
                    background: state === 'done' ? 'var(--signal-ok)' : state === 'active' ? 'var(--ice)' : 'var(--ink-500)',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ice)' }}>{Math.round(simProgress)}%</span>
        ...
        {/* Pedagogical text */}
        <div style={{ padding: '10px 12px', background: 'var(--ice-dim)', border: '1px solid var(--ice-border)', borderRadius: 'var(--radius-sm)', marginBottom: 14 }}>
          <div style={{ fontSize: 8, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--ice)', fontFamily: 'var(--font-ui)', marginBottom: 4 }}>
            What the model is doing
          </div>
          <p style={{ fontFamily: 'var(--font-ui)', fontSize: 10, color: 'var(--text-mid)', lineHeight: 1.5, fontStyle: 'italic' }}>
            "{SIM_MESSAGES[msgIdx % SIM_MESSAGES.length]}"
          </p>
          <div style={{ display: 'flex', gap: 4, marginTop: 6, justifyContent: 'center' }}>
            {SIM_MESSAGES.map((_, i) => (
              <div key={i} style={{
                width: 5, height: 5, borderRadius: '50%',
                background: i === msgIdx % SIM_MESSAGES.length ? 'var(--ice)' : 'var(--ink-500)',
        ...
        {/* Scenario recap */}
        <div style={{ padding: '8px 10px', background: 'var(--ink-500)', borderRadius: 'var(--radius-sm)', marginBottom: 14 }}>
